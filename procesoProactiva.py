...
def procesoProactiva(fechaInput, archivoXlsxInput, archivoComplementoCliente, pathArchivoTxt, pathArchivoXlsx):
    ...
    try:
        ...
        salidaTxt = "{0}/{1}{2}.txt".format(pathArchivoXlsx, formatoSalidaTxt, fechaInput)
...
def main():
    if len(sys.argv) == PROCESOS_GENERALES['PROACTIVA']['ARGUMENTOS_PROCESO'] + 1:
        ...
        pathArchivoXlsx = str(sys.argv[5])
        pathNoEncontrado = []
        directorioNumero = []
        pathNoPermisos = []
        ...
        if archivosValidos and encabezadosValidos:
            procesoProactiva(fechaEntrada, archivoProactivaXls, archivoComplmentoCliente, pathArchivoTxt, pathArchivoXlsx)
        else:
